...
class googlecalendar_extension(commands.Cog):
    ...
    async def gcal_notifier(self, ctx):
        ...
        for dbdata in calendar_dbdata:
            eventdata = self.google_apiresponse_sorter(self.google_api_get_events(168, dbdata[0])) # ToDo: run this in executor so the bot doesnt slow down
